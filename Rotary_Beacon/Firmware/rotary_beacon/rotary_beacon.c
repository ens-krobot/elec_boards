...
static void NORETURN sender_process(void) {
    ...
    for (;;) {
        ...
        f.data32[0] = pos.d[0];
        f.data32[1] = pos.d[1];
        can_transmit(CAND1, &f, ms_to_ticks(20));
        ...
        f.dlc = sizeof(pos_ll.p);
        f.eid = 302;
        f.data32[0] = pos_ll.d[0];
        f.data32[1] = pos_ll.d[1];
        can_transmit(CAND1, &f, ms_to_ticks(20));
    }
}
...
void beacon_init(void) {
    ...
    proc_new(beacon_update_process, NULL, KERN_MINSTACKSIZE * 4, NULL);
    proc_new(sender_process, NULL, KERN_MINSTACKSIZE * 4, NULL);
}
