...
#define LED2_PIN                    BV(0)
#define LED3_PIN                    BV(8)
#define LED4_PIN                    BV(12)

#define LED1_GPIO_BASE          ((struct stm32_gpio *)GPIOA_BASE)
#define LED2_GPIO_BASE          ((struct stm32_gpio *)GPIOB_BASE)
#define LED3_GPIO_BASE          ((struct stm32_gpio *)GPIOC_BASE)
#define LED4_GPIO_BASE          ((struct stm32_gpio *)GPIOC_BASE)
...
#define LED3_ON()							\
		do {							\
			stm32_gpioPinWrite(LED3_GPIO_BASE, LED3_PIN, 1);\
		} while (0)
#define LED3_OFF()							\
		do {							\
			stm32_gpioPinWrite(LED3_GPIO_BASE, LED3_PIN, 0);\
		} while (0)

#define LED4_ON()							\
		do {							\
			stm32_gpioPinWrite(LED4_GPIO_BASE, LED4_PIN, 1);\
		} while (0)
#define LED4_OFF()							\
		do {							\
			stm32_gpioPinWrite(LED4_GPIO_BASE, LED4_PIN, 0);\
		} while (0)
...
#define LEDS_INIT()							\
		do {							\
			...
			stm32_gpioPinConfig(LED2_GPIO_BASE,		\
			    ...
			    GPIO_SPEED_50MHZ);		                \
			stm32_gpioPinConfig(LED3_GPIO_BASE,		\
			    LED3_PIN, GPIO_MODE_OUT_PP,                 \
			    GPIO_SPEED_50MHZ);		                \
			stm32_gpioPinConfig(LED4_GPIO_BASE,		\
			    LED4_PIN, GPIO_MODE_OUT_PP,                 \
			    GPIO_SPEED_50MHZ);		                \
		} while(0)
