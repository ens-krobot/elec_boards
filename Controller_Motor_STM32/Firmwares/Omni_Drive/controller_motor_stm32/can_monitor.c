...
static void NORETURN canMonitor_process(void) {
  ...
  while(1) {
    ...
    msg_enc.data.encoder1_pos = getEncoderPosition(ENCODER1);
    msg_enc.data.encoder2_pos = getEncoderPosition(ENCODER2);
    msg_enc.data.encoder1_dir = getEncoderDirection(ENCODER1);
    msg_enc.data.encoder2_dir = getEncoderDirection(ENCODER2);

    txm.data32[0] = msg_enc.data32[0];
    txm.data32[1] = msg_enc.data32[1];
    txm.eid = CAN_MSG_ENCODERS12;
    can_transmit(CAND1, &txm, ms_to_ticks(10));
      ...
      if (received) {
        if (frame.rtr == 1) {
          ...
          case CAN_MSG_MOVE_X:
            ...
            if (!tc_is_working(TC_MASK(HD_LINEAR_SPEED_X_TC)))
              hd_move_X(move_msg.data.distance / 1000.0, move_msg.data.speed / 1000.0, move_msg.data.acceleration / 1000.0);
            break;
          case CAN_MSG_MOVE_Y:
            move_msg.data32[0] = frame.data32[0];
            move_msg.data32[1] = frame.data32[1];
            if (!tc_is_working(TC_MASK(HD_LINEAR_SPEED_Y_TC)))
              hd_move_Y(move_msg.data.distance / 1000.0, move_msg.data.speed / 1000.0, move_msg.data.acceleration / 1000.0);
            break;
          case CAN_MSG_TURN:
            turn_msg.data32[0] = frame.data32[0];
            turn_msg.data32[1] = frame.data32[1];
            if (!tc_is_working(TC_MASK(HD_ROTATIONAL_SPEED_TC)))
              hd_turn(turn_msg.data.angle / 10000.0, turn_msg.data.speed / 1000.0, turn_msg.data.acceleration / 1000.0);
            break;
          ...
          case CAN_MSG_DRIVE_ACTIVATION:
            ...
            if (!drive_activation_msg.data.activate) {
              ...
              hd_set_linear_speed_X(0., 0.);
              hd_set_linear_speed_Y(0., 0.);
          ...
          case CAN_MSG_STOP:
            ...
            hd_set_linear_speed_X(0., stop_msg.data.lin_acc);
            hd_set_linear_speed_Y(0., stop_msg.data.lin_acc);
            hd_set_rotational_speed(0., stop_msg.data.rot_acc);
          ...
          case CAN_MSG_SIMULATION_MODE:
            ...
            case SIMULATION_MODE_NORMAL:
              mc_change_mode(MOTOR2, CONTROLLER_MODE_HIL);
            ...
            case SIMULATION_MODE_HIL:
              ...
              break;
            default:
              mc_change_mode(MOTOR2, CONTROLLER_MODE_NORMAL);
