...
//    OpenI2C(MASTER, SLEW_OFF);
    ...
    */
    ...
    OpenTimer1(TIMER_INT_OFF      // désactivation des interruption par dépassement du Timer (Timer overflow)
...
*/
void ProcessIO(void) {   
    static char rangefinder = 0;
    static char mode = MODE_CALIBRATE_AUTO;
    static char status;
    static BOOL skipMeas;
    static int nbMeas = 0;
    ...
    switch (mode) {
        case MODE_MEASURE:
            if (measureDistance(rangefinder)) {
                if (rangefinder >= 7) {
                    rangefinder = 0;

                    // Recalibration automatique toutes les X mesures.
                    // Les mesures ne sont pas valides durant la calibration,
                    // dont on ne connait pas l'instant précis de déclenchement.
...
*/
        ...
        case MODE_CALIBRATE:
            if (!HIDTxHandleBusy(USBInHandle)) {
                if (status == CAL_START || status == CAL_CONTINUE) {
                    status = calibrate(rangefinder, skipMeas);
    
                    if (status != CAL_CONTINUE) {
                        ToSendDataBuffer.HSEQ    = 0;                        // Numéro séquence PC
                        ToSendDataBuffer.DSEQ    = (UPClientSeq++);            // Numéro séquence PIC
                        ToSendDataBuffer.CMD    = CMD_CALIBRATE;            // Type requête
                        ToSendDataBuffer.ERR    = 0;                        // Erreur

                        ToSendDataBuffer.DATA[0] = status;
                        ToSendDataBuffer.DATA[1] = ReadEEPROM(8*rangefinder + 1);
                        ToSendDataBuffer.DATA[2] = ReadEEPROM(8*rangefinder + 2);
                        ToSendDataBuffer.DATA[3] = ReadEEPROM(8*rangefinder + 3);
                        ToSendDataBuffer.DATA[4] = ReadEEPROM(8*rangefinder + 4);
                        ToSendDataBuffer.DATA[5] = ReadEEPROM(8*rangefinder + 5);
                        ToSendDataBuffer.DATA[6] = ReadEEPROM(8*rangefinder + 6);
                        ToSendDataBuffer.DATA[7] = ReadEEPROM(8*rangefinder + 7);
                        ToSendDataBuffer.DATA[8] = ReadEEPROM(8*rangefinder + 8);
                        USBInHandle = HIDTxPacket(HID_EP, (BYTE*) &ToSendDataBuffer, 64);

                        if (status == CAL_ERROR || status == CAL_DONE)
                            mode = MODE_MEASURE;
                    }
                }
            }
        ...
        case MODE_CALIBRATE_AUTO:
            status = calibrateAuto(rangefinder);
                ...
                if (!HIDTxHandleBusy(USBInHandle)) {
                    ...
                    switch (ReceivedDataBuffer.DATA[0]) {
                        ...
                        case GET_BOARD_INFO:
                            ...
                            strcatpgm2ram((char *) &ToSendDataBuffer.DATA[0], (const rom far char *) "Carte capteurs 1.0\nOlivier BICHLER");
                        break;
                        ...
                        case GET_RANGEFINDER_CALIBRATION:
/*
                            ToSendDataBuffer.DATA[0] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 1);
                            ToSendDataBuffer.DATA[1] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 2);
                            ToSendDataBuffer.DATA[2] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 3);
                            ToSendDataBuffer.DATA[3] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 4);
                            ToSendDataBuffer.DATA[4] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 5);
                            ToSendDataBuffer.DATA[5] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 6);
                            ToSendDataBuffer.DATA[6] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 7);
                            ToSendDataBuffer.DATA[7] = ReadEEPROM(8*ReceivedDataBuffer.DATA[1] + 8);
*/
                            ToSendDataBuffer.DATA[0] = CalibrationData[8*ReceivedDataBuffer.DATA[1]];
                            ToSendDataBuffer.DATA[1] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+1];
                            ToSendDataBuffer.DATA[2] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+2];
                            ToSendDataBuffer.DATA[3] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+3];
                            ToSendDataBuffer.DATA[4] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+4];
                            ToSendDataBuffer.DATA[5] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+5];
                            ToSendDataBuffer.DATA[6] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+6];
                            ToSendDataBuffer.DATA[7] = CalibrationData[8*ReceivedDataBuffer.DATA[1]+7];
                        break;

                        case GET_TOR_STATE:
                            ToSendDataBuffer.DATA[0] = (TOR8 << 7) | (TOR7 << 6) | (TOR6 << 5) | (TOR5 << 4) | (TOR4 << 3) | (TOR3 << 2) | (TOR2 << 1) | TOR1;
                            ToSendDataBuffer.DATA[1] = (TOR16 << 7) | (TOR15 << 6) | (TOR14 << 5) | (TOR13 << 4) | (TOR12 << 3) | (TOR11 << 2) | (TOR10 << 1) | TOR9;
                        break;
            ...
            case CMD_CALIBRATE:
                switch (ReceivedDataBuffer.DATA[0]) {
                    case CAL_START:
                        ...
                        mode = MODE_CALIBRATE;
                        rangefinder = ReceivedDataBuffer.DATA[1];
                        skipMeas = (BOOL) ReceivedDataBuffer.DATA[2];
                        status = CAL_START;
                    break;

                    case CAL_CONTINUE:
                        // Continue la calibration après une demande d'action de l'utilisateur
                        status = CAL_CONTINUE;
                    break;
