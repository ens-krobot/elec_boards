/**
 * @mainpage Battery Monitoring
 * @section Introduction
 * Ce firmware est destiné à la carte Battery Monitoring, équipée d'un PIC18F4550.
 * Le programme déclenche une alarme (buzzer) lorsque la tension au borne d'une cellule
 * de la batterie est trop faible. Si les connecteurs d'équilibrage de la batterie ne
 * sont pas connectés correctement sur la carte, un bip sonnore sera produit, jusqu'à ce
 * que les cellules de la batterie soient connectés et soient dans un état de charge suffisant.
 *
...
/** VECTOR REMAPPING ***********************************************/
    ...
    void YourHighPriorityISRCode()
    {
        ...
        if (PIR1bits.TMR1IF) {
            WriteTimer1(55536); // 2^16 - 10 000 : la prochaine interruption a lieu dans 10 000 cycles = 10 ms
            ms+= 10;
...
void UserInit(void)
{
    ...
    OpenTimer1(TIMER_INT_ON        // active le timer1
         ...
         & T1_PS_1_8            // incrémente le compteur tous les 8 cycles
                ...
                if (!HIDTxHandleBusy(USBInHandle)) {
                    ...
                    switch (ReceivedDataBuffer.DATA[0]) {
                        ...
                        case GET_BOARD_INFO:
                            ...
                            strcatpgm2ram((char *) &ToSendDataBuffer.DATA[0], (const rom far char *) "Battery Monitoring 1.0\nOlivier BICHLER");
                        break;
                        ...
                        case GET_CELL_VOLTAGE:
                            for (i = 0; i < NB_CELLS; i++) {
                                word.Val = getCellVoltage(i);
                                ToSendDataBuffer.DATA[2*i]    = word.byte.HB;
                                ToSendDataBuffer.DATA[2*i+1]    = word.byte.LB;
                            }
                        break;

                        case GET_CURRENT:
                            dword.Val = getCurrent();
                        ...
                        case GET_POWER_STATE:
                            ToSendDataBuffer.DATA[0] = getPowerState();
                        break;

                        case GET_BATTERY_STATE:
                            ToSendDataBuffer.DATA[0] = getBatteryState();
                        break;
...
/**
 * Fonction d'interruption du buzzer
 */
void interruptBuzzer(void) {
	static WORD buzzer_count = 0;
    ...
    if (buzzer_count == 0) {
        switch (getBatteryState()) {
            case 0:
                // Au moins une cellule n'est pas connectée
                // /!\ d'autres peuvent être sous le seuil de tension faible
                // SIRENE INTERMITTENTE
                BUZZER = !BUZZER;
            break;

            case 1:
                // Au moins une cellule est sous le seuil de tension faible
                // SIRENE CONTINUE
                BUZZER = 1;
            break;
