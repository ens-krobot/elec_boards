...
export  comp_SetPower
export _comp_SetPower
...
;
   ...
   mov  A, reg[comp_COMP_CR0]                    ; read old CR0 value
   and  A, ~REFMASK                             ; clear reference bits in A
   tst  [X],HIGHGAIN                            ; See if High Gain bit is set
   jnz  .SETHIGHGAIN
   and  reg[comp_COMP_CR3],~EXGAIN               ; Clear High Gain bit.
   or   A, [X]                                  ; combine gain value with balance of reg.
   mov  reg[comp_COMP_CR0], A                    ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   mov  reg[comp_COMP_CR0], A       ; move complete value back to register
   or   reg[comp_COMP_CR3], EXGAIN               ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
    ret
.ENDSECTION
...
_comp_Stop:
   ...
   and reg[comp_COMP_CR2], FCh
   RAM_EPILOGUE RAM_USE_CLASS_1
    ret
.ENDSECTION

; End of File comp.asm
