...
export  PGA_3_Start
export _PGA_3_Start
export  PGA_3_SetPower
export _PGA_3_SetPower

export  PGA_3_SetGain
export _PGA_3_SetGain
...
;
   ...
   mov  A, reg[PGA_3_GAIN_CR2]                             ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[PGA_3_GAIN_CR2], A                        ; move complete value back to register
   pop  A
...
;
   ...
   mov  A, reg[PGA_3_GAIN_CR0]                             ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[PGA_3_GAIN_CR3],~EXGAIN                        ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_3_GAIN_CR0], A                             ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_3_GAIN_CR0], A                             ; move complete value back to register
   or   reg[PGA_3_GAIN_CR3], EXGAIN                        ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_PGA_3_Stop:
   ...
   and REG[PGA_3_GAIN_CR2], ~POWERMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
; End of File PGA_3.asm
